...
<html lang="pt">
	<head>
		...
		<title>PS3 Toolset MOD Genesis Games ( By Carlos Tuga )</title>
      ...
      var get_day =function() {return '<?php echo(date('w')); ?>';};
			...
			var updateErrorDetails = function (dtext,err){
				...
				$.ajax({
					...
					data==='OK' ? Logger.info('Sessão de GC encerrada') : Logger.error('Sessão de GC encerrada');
				}).fail(function() {
					Logger.error('Falha na sessão de GC');
				});
			...
			$('.refresh-fm').click(function(){
				...
				$('.preloader').removeClass('ui-helper-hidden');
				setTimeout(function(){
					tabreload('flashmem',toast('Reinicie o gerenciador de flash. Aguarde...','aviso',120));
				},100);
			});
			$('.refresh-me').click(function(){
				...
				$('.preloader').removeClass('ui-helper-hidden');
				setTimeout(function(){
					tabreload('memedit',toast('Recarregando o editor de memória Userland. Aguarde...','aviso',120));
				},100);
			});
			$('.refresh-fe').click(function(){
				...
				$('.preloader').removeClass('ui-helper-hidden');
				setTimeout(function(){
					tabreload('fileman',toast('Reinicie o gerenciador de arquivos. Aguarde...','aviso',120));
				},100);
			});
			function tabreload(name,tost){
				...
				setTimeout(function(){
					$.ajax({
						...
						if(data.length===0){Logger.error('Erro ao carregar o arquivo de recurso');return;}
						var o = $('#'+name);
						var par = o.parent();
						if(name==='memedit'){
			...
			$('#tabs').removeClass('ui-helper-hidden').tabs({
				...
				create: function( event, ui ){
					...
					if(get_day()!== cdate.getUTCDay().toString() || get_year() !== cdate.getUTCFullYear().toString()){
						updateErrorDetails('Este projeto requer a configuração correta do relógio do PS3.','Erro de verificação de hora do sistema. Por favor, ajuste o relógio do sistema.');
						return;
					}
					else{
						$.ajax({
							...
							if(data.length===0){updateErrorDetails('Falha ao carregar a estrutura de exploração do PS3.','Erro ao carregar arquivo de biblioteca de números inteiros');return;}
							eval(data);
							$.ajax({
								url: 'file3.php',
								method: 'POST',
								...
								if(data.length===0){updateErrorDetails('Falha ao carregar a estrutura de exploração do PS3.','Erro ao carregar o arquivo da biblioteca da estrutura de exploração');return;}
								eval(data);
								if(jsleak32(0x10000)!==0x7F454C46){
									updateErrorDetails('O console não é um modelo CEX/DEX PS3.','Console incompatível detectado');
									return;
								}
								var fpwait = 0;
								function compload(){
									...
									if(fp9loaded===false){
										if(fpwait<16){
											if(fpwait===1){Logger.warn('Aguardando o plug-in PS3 Flash Player 9...');}
											setTimeout(compload,1000);
										}
										else{
											updateErrorDetails('PS3 Toolset não pôde carregar o arquivo SWF','Se seu navegador não solicitar que você carregue o plug-in do Flash, há 2 motivos possíveis: uma conexão de Internet lenta/não confiável que impede que alguns arquivos sejam recebidos em um maneira oportuna, ou o plug-in do Flash Player pode ter sido desativado permanentemente neste perfil de usuário, se for o caso, você precisará entrar em um perfil de usuário diferente OU excluir o arquivo settings.xml na pasta do navegador da web do perfil atual se você estiver no CFW /HEN.');
											toast('Para usar o PS3 Toolset, você deve concordar em baixar o plug-in PS3 Flash Player 9 se a caixa de diálogo de confirmação do plug-in do navegador aparecer.<br/>Verifique o Histórico para obter mais informações.','warning',7);
											setTimeout(function(){
												setTimeout(function(){
													$('#dg-confirm').parent().find('.ui-dialog-buttonpane').find('button:last').focus();
												},750);
												confirmDialog('O PS3 Toolset agora tentará reiniciar. Deseja continuar?','Atualização do conjunto de ferramentas',function(){location.reload();});
											},5200);
										}
										return;
									}
									else if(navigator.plugins.length===0){
										updateErrorDetails('O PS3 Toolset requer que o plug-in do Flash Player 9 esteja ativado.','Se seu navegador não solicitar o download do plug-in do Flash, há 2 motivos possíveis: uma conexão de internet lenta/não confiável que impede alguns os arquivos podem ser obtidos a tempo ou o plug-in do Flash Player pode ter sido desativado permanentemente neste perfil de usuário. Nesse caso, você precisará fazer login em um perfil de usuário diferente OU excluir o arquivo settings.xml no navegador da Web do perfil atual pasta se você estiver em CFW /HEN.');
										toast('Para usar o PS3 Toolset, você deve concordar em baixar o plug-in PS3 Flash Player 9 se a caixa de diálogo de confirmação do plug-in do navegador aparecer.<br/>Verifique o Histórico para obter mais informações.','warning',7);
										return;
									}
									else{
								...
								updateErrorDetails('Falha ao carregar a estrutura de exploração do PS3.','Erro ao carregar o arquivo da biblioteca da estrutura de exploração');
							});
						}).fail(function(data) {
							updateErrorDetails('Falha ao carregar a estrutura de exploração do PS3.','Erro ao carregar arquivo de biblioteca de números inteiros');
						});
					}
				},
	...
	<body id="BodyID" class="ui-helper-hidden" style="overflow: hidden;height:auto;visibility:hidden;">
		<div class="preloader ui-helper-hidden"><div class="container-busy-icon"><div class="busy-icon"></div></div></div>
			<div id="title" class="ui-helper-hidden main-title ui-widget-header ui-corner-all">
				<h1>PlayStation 3 Toolset MOD <span class='header-small-text'>Genesis Games ( By Carlos Tuga )</span></h1>
				<h4 id='ps3details' class="ps3-details">Inicialização PS3 Toolset MOD <span class='header-small-text'>build 004</span><br/>Por favor, espere...</h4>
				<form action="#">
					<select id="themes" >
						<option value="dummy" disabled selected>Temas</option>
						<option value="sunny" >Ensolarado</option>
						<option value="eggplant" >Beringela</option>
						<option value="hot-sneaks">Trópico</option>
			...
			<div id="tabs" class='ui-helper-hidden main-tabs ' style='height:780px;min-height:780px;'>
				<ul>
					<li><a href='#toolset'><i class="fa fa-home fa-fw"></i> Casa</a></li>
					<li><a href='memedit.php'><i class="fa fa-table fa-fw"></i> Editor de memória<span title='Atualizar guia do Editor de memória' class='refresh fa fa-refresh ui-state-disabled refresh-me pointer tab-icon'></span></a></li>
					<li><a href='flashmem.php'><i class="fa fa-microchip fa-fw"></i> Flash Manager<span title='Atualizar a guia Gerenciador de Flash' class='refresh fa fa-refresh ui-state-disabled refresh-fm pointer tab-icon'></span></a></li>
					<li><a href='fileman.php'><i class="fa fa-table fa-hdd-o"></i> Gerenciador de arquivos (em breve)<span title='Atualizar a guia Gerenciador de Arquivos' class='refresh fa fa-refresh ui-state-disabled refresh-fe pointer tab-icon'></span></a></li>
					<li><a href='#tblog'><i class="fa fa-list-alt fa-fw"></i> Histórico</a></li>
				</ul>
				<div id="toolset">
					<h2 align='right' class='tab-header'>PS3 Toolset MOD Genesis Games ( By Carlos Tuga )<span class='header-tiny-text'>v1.1.004</span></h2>
						...
						<div class='box-table' style="max-height:620px;min-height:600px;height:620px;">
										...
										<tr class="window-header ui-widget-header">
											<th class="logoptions window-header ui-widget-header">
												<div class="nopad">
													...
													<span class='top2px baloo-header'> Bem-vindo</span>
												</div>
											</th>
										</tr>
										<tr class="logoptions window-content-top ui-widget-content">
											<td id='intr' align="justify" class="window-content-top">
												<div class='sizer'>
													...
													PS3 Toolset é um repositório de ferramentas baseado na estrutura de exploração 4.1 para PS3.<br/>
													Novas ferramentas e funcionalidades serão adicionadas ao longo do tempo.<br/>
													Esperamos que você goste de usar sua funcionalidade.
										...
										<tr class="window-header ui-widget-header">
											<th class="logoptions window-header ui-widget-header">
												<div class="nopad">
													...
													<span class='top2px baloo-header'> Confidencialidade</span>
												</div>
											</th>
										</tr>
										<tr class="logoptions window-content-top ui-widget-content">
											<td id='security' align="justify" class="window-content-top">
												<div class='sizer'>
													Este site não coleta ou armazena nenhuma informação pessoal ou técnica relacionada a você ou ao seu console.<br/>
													Nenhum dado é enviado para o nosso servidor web ao usar o PS3 Toolset, todas as operações são feitas localmente.<br/>
													Os cookies são usados ​​localmente no PS3 para salvar várias variáveis ​​do PS3 Toolset de uma sessão para outra.
												</div>
											</td>
										</tr>
										<tr class="pl window-bottom-small">
							...
							<div class='width-600 box-cell-70' >
								...
								<div>
									...
									<br/><br/>
										10/06/2022 Atualizar v1.1.004
										<ul class="fa-ul">
											<li><i class="fa-li fa fa-chevron-circle-right"></i>Adicionado suporte para 4.89 CEX<br/></li>
											<li><i class="fa-li fa fa-chevron-circle-right"></i>Flash NC exploit atualização v3.0<br/></li>
											<li><i class="fa-li fa fa-chevron-circle-right"></i>Atualização do Gerenciador de Memória Flash v1.3.1<br/></li>
											<li><i class="fa-li fa fa-chevron-circle-right"></i>Atualização da estrutura JavaScript v4.2<br/></li>
										</ul>
									</div>
									<br/>
									<div align='right' class='wrap-don'>
										<i class="fa fa-border fa-quote-left fa-fw" style="font-size:8px;"></i>
										<span style="font-size:11px;font-style:italic;">Apenas um pequeno lançamento!</span>
								...
								<div>
									<div align='left' class='wrap-don'>
										<ul class="fa-ul">
											<li><i class="fa-li fa fa-chevron-circle-right"></i>Você pode usar as ferramentas deste projeto por sua conta e risco.
Observe que nenhum suporte oficial é fornecido. Se você tiver algum problema e precisar de ajuda, recomendamos fortemente que entre em contato <a href="" title="PlayStation 3 Toolset MOD Genesis Games">( By Carlos Tuga )</a> para apoio e aconselhamento.</li>
											<li><i class="fa-li fa fa-chevron-circle-right"></i>O plug-in do navegador Flash Player 9 deve estar ativado para usar o PS3 Toolbox.<br/>
											Se você já o desativou permanentemente em seu perfil de usuário atual, pode ser necessário fazer login como um usuário diferente ou criar um novo perfil para poder usar qualquer uma das ferramentas deste projeto.</li>
											<li><i class="fa-li fa fa-chevron-circle-right"></i>Você pode ativar o Flash permanentemente marcando a caixa de seleção "Não mostrar novamente" na tela de confirmação do plug-in antes de concordar em baixar o plug-in do Flash.</li>
											<li><i class="fa-li fa fa-chevron-circle-right"></i>É altamente recomendável que você defina as configurações de horário do sistema do console corretamente para evitar problemas relacionados ao horário no navegador e/ou no plug-in do Flash Player.</li>
											<li><i class="fa-li fa fa-chevron-circle-right"></i>Para evitar possíveis falhas, nunca tente fechar o navegador durante a execução de operações da caixa de ferramentas, especialmente se a configuração de confirmação de saída do navegador estiver desativada.</li>
										</ul>
									</div>
								</div>
								...
								<div>
									<div align='left' class='wrap-don'>
										<ul class="fa-ul" style="line-height:22px;">
											<li><i class="fa-li fa fa-chevron-circle-right" style="line-height:18px;"></i>Plug-in do navegador PS3 Flash Player 9 incluído<br/></li>
											<li><i class="fa-li fa fa-chevron-circle-right" style="line-height:18px;"></i>Navegador PS3 ativado Javascript<br/></li>
											<li><i class="fa-li fa fa-chevron-circle-right" style="line-height:18px;"></i>Cookies do navegador PS3 ativados<br/></li>
											<li><i class="fa-li fa fa-chevron-circle-right" style="line-height:18px;"></i>Firmware PS3: 4.89<br/></li>
											<li><i class="fa-li fa fa-chevron-circle-right" style="line-height:18px;"></i>Para firmware 4.80-4.88, vá para: <a href="https://www.ps3xploit.net/bgtoolset/" title="PlayStation 3 Toolset by @bguerville">PlayStation 3 Toolset by @bguerville</a><br/></li>
											<li><i class="fa-li fa fa-chevron-circle-right" style="line-height:18px;"></i>Tipos de firmware PS3: OFW/HFW/MFW/CFW<br/></li>
											<li><i class="fa-li fa fa-chevron-circle-right" style="line-height:18px;"></i>Modos de firmware do PS3: CEX/DEX<br/></li>
											<li><i class="fa-li fa fa-chevron-circle-right" style="line-height:18px;"></i>Hora do sistema PS3 definida com precisão<br/></li>
										</ul>
									</div>
								</div>
								<h3> Obrigado</h3>
								<div>
									<div class='wrap-don'>
										<p>Agradeço sinceramente a Jason por sua amizade e apoio, é claro, mas no contexto deste projeto, e também por testar este trabalho durante todo o ano, conforme necessário.<br/></p>
										<br/>
										<p>O PS3 Toolkit e sua GUI foram construídos com js com base em várias bibliotecas js de código aberto, incluindo jQuery, jQueryUI, bigInteger, jstree, mCustomScrollbar, js-logger, js-cookie, sjcl, switchButton e toastmessage, bem como a biblioteca de ícones Fork Awesome CSS.<br/>Obrigado a todos os codificadores envolvidos em vários projetos.</p>
										<br/>
										<p>Obrigado aos hackers da cena ps3/vita, desenvolvedores, criadores de fóruns e colaboradores do psdevwiki que nos ajudaram a chegar a este ponto.</p>
									</div>
								</div>
								<h3> Ajuda e suporte</h3>
								<div>
								<div class='wrap-don'>
										Em nome da equipe e dos usuários da ferramenta PS3Xploit, gostaria de expressar nossa sincera gratidão pelo suporte até hoje, seus testes e conselhos permitiram que a equipe respondesse a tempo e melhorasse nosso potencial.<br/>
										Precisamos do seu constante apoio moral no desenvolvimento do projeto, que estamos desenvolvendo gratuitamente e sem publicidade.
                                        Se você quiser nos ajudar em nossa difícil tarefa, participe ativamente dos testes e ficaremos muito gratos a você!<br/><br/>
										</div>
									</div>
								</div>
							</div>
			...
			<div id='tblog' class="tb-log" style="max-height:90%;">
				<h2 align='right'  class='tab-header'>Histórico <span class='header-tiny-text'>v1.1</span></h2>
									...
									<div class="dir-table-auto" style="max-height:25px;height:25px;font-size:12px;">
										<span class='min-width-410 dir-left' style="min-width:600px;width:600px;padding-left:0;">
											<span class='sizer'>
												...
												<label id="lilog" for="ilog" title="Histórico" class="logbtn">Histórico</label>
												<input type='checkbox' class="ui-widget gui-item" id='iwarn' name='iwarn' checked />
												<label id="liwarn" for="iwarn" title="Avisos" class="logbtn">Avisos</label>
												<input type='checkbox' id='ierror' name='ierror' class='gui-item' checked  />
												<label id="lierror" for="ierror" title="Erros" class="logbtn">Erros</label>
												<input type='checkbox' id='idbg' name='idbg' class='gui-item' />
												<label id="lidbg" for="idbg" title="Registros do depurador da caixa de ferramentas" class="logbtn">Depurar mensagens</label>
												<span style="padding-left:20px;font-size:8px;">
													...
													<span style="padding-left:5px;font-size:10px;"> página do logotipo: </span>
												...
												<span style="padding-left:20px;">
													...
													<label for="inet" title="O depurador da caixa de ferramentas registra UDP" >UDP Broadcast</label>
													<label class='labport' for="port_txtbox" style="padding-left:5px;"> Porta: </label>
